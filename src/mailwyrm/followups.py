...
from mailwyrm.models import FollowUpMarker, ReadLaterMarker
...
def set_read_later(
    ...
    *,
    ...
    if read_later:
        created_at = datetime.now(UTC).isoformat()
        for message_id in unique_message_ids:
            if message_id in state.read_later:
                continue
            state.read_later[message_id] = ReadLaterMarker(
                message_id=message_id,
                reason=reason,
                created_at=created_at,
            )
        ...
        for message_id in unique_message_ids:
            if state.read_later.pop(message_id, None) is not None:
                changed += 1

    return {
        "changed": changed,
        "marked": sum(1 for message_id in unique_message_ids if message_id in state.read_later),
        "total": len(unique_message_ids),
    }


def message_marked_read_later(state: MailwyrmState, message_id: str) -> bool:
    return message_id in state.read_later
